...
function mas_pdf_clean($value): string
{
...
function mas_pdf_date($date): string
{
...
function mas_pdf_time($time): string
{
...
function mas_pdf_is_super_admin(mysqli $conn): bool
{
...
$direct = mysqli_query(
    $conn,
    "SELECT id FROM mas_main WHERE id = $viewId LIMIT 1"
);
    ...
    if ($select) {
        ...
        if ($submission) {
            foreach (['report_reference_id', 'source_id', 'reference_id'] as $column) {
                ...
                $check = mysqli_query(
                    $conn,
                    "SELECT id FROM mas_main WHERE id = $candidate LIMIT 1"
                );
...
$canAccess =
    mas_pdf_is_super_admin($conn)
    || (int)($mas['manager_employee_id'] ?? 0) === $employeeId
    || (int)($mas['team_lead_employee_id'] ?? 0) === $employeeId
    || (int)($mas['created_by'] ?? 0) === $employeeId;

if (!$canAccess) {
    die('MAS not found or access denied.');
}
...
$stmt = mysqli_prepare($conn, "
    ...
    FROM mas_details
    WHERE mas_main_id = ?
      AND TRIM(COALESCE(attendee_name, '')) <> ''
    ORDER BY sl_no, id
");
...
class MASCURRENT_PDF extends FPDF
{
    public array $meta = [];
    public string $logoPath = '';
    public string $fontFamily = 'Arial';
    public array $widths = [15, 80, 60, 40, 55, 27];
    ...
    public function Header(): void
    {
        ...
        $meetingHeld =
            ($this->meta['meeting_venue'] ?? '')
            . ' / '
            . ($this->meta['meeting_date'] ?? '');
        ...
        $metaRows = [
            ...
            ['PMC', $this->meta['pmc'] ?? ''],
            ['Meeting Held/Dated', $meetingHeld],
        ];
        ...
        $this->Cell(
            ...
            7,
            'Meeting: '
            . mas_pdf_clean($this->meta['meeting_title'] ?? '')
            . ' | Time: '
            . mas_pdf_clean($this->meta['meeting_time'] ?? '')
            . ' | Conducted By: '
            . mas_pdf_clean($this->meta['conducted_by'] ?? ''),
            1,
            1,
            'L'
        );
    ...
    public function Footer(): void
    {
        ...
        $this->Cell(
            0,
            10,
            mas_pdf_clean($this->meta['company_name'] ?? ''),
            0,
            0,
            'L'
        );
...
$pdf = new MASCURRENT_PDF('L', 'mm', 'A4');
$pdf->initialiseFonts();
...
$pdf->meta = [
    ...
    'project_name' => $mas['project_name'] ?? '',
    'client_name' => $mas['client_name']
        ?: ($mas['current_client_name'] ?? ''),
    'architect' => $mas['architect'] ?? '',
    'pmc' => $mas['pmc'] ?? '',
    'meeting_title' => $mas['meeting_title'] ?? '',
    'meeting_date' => mas_pdf_date($mas['meeting_date'] ?? ''),
    'meeting_time' => mas_pdf_time($mas['meeting_time'] ?? ''),
    'meeting_venue' => $mas['meeting_venue'] ?? '',
    'conducted_by' => $mas['conducted_by'] ?? '',
];
...
$filename = 'MAS_'
    . preg_replace(
        '/[^A-Za-z0-9_-]/',
        '_',
        mas_pdf_clean($mas['mas_no'] ?? ('ID_' . $viewId))
    )
    . '.pdf';
