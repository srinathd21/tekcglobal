...
function dds_pdf_clean($value): string
{
...
function dds_pdf_date($date): string
{
...
function dds_pdf_is_super_admin(mysqli $conn): bool
{
...
class DDSCURRENT_PDF extends FPDF
{
    public array $meta = [];
    public string $logoPath = '';
    public string $fontFamily = 'Arial';
    public array $widths = [18, 110, 30, 30, 30, 59];
    ...
    public function Header(): void
    {
        ...
        $metaRows = [
            ...
            ['Structural Consultant', $this->meta['structural_consultant'] ?? ''],
            ['PMC', $this->meta['pmc'] ?? ''],
            ['Date / Version', ($this->meta['dds_date'] ?? '') . ' / ' . ($this->meta['version'] ?? '')],
        ];
    ...
    public function Footer(): void
    {
        ...
        $this->Cell(
            0,
            10,
            dds_pdf_clean($this->meta['company_name'] ?? ''),
            0,
            0,
            'L'
        );
...
$pdf = new DDSCURRENT_PDF('L', 'mm', 'A4');
$pdf->initialiseFonts();
...
$pdf->meta = [
    ...
    'architect' => $dds['architect'] ?? '',
    'structural_consultant' => $dds['structural_consultant'] ?? '',
    'pmc' => $dds['pmc'] ?? '',
    'dds_date' => dds_pdf_date($dds['dds_date'] ?? ''),
    'version' => $dds['version'] ?? 'R0',
];
...
foreach ($rows as $row) {
    ...
    $pdf->drawDataRow(
        [
            ...
            dds_pdf_date($row['start_date'] ?? ''),
            dds_pdf_date($row['end_date'] ?? ''),
            $row['remarks'] ?? '',
        ],
        ['C', 'L', 'C', 'C', 'C', 'L']
    );
...
$filename = 'DDS_'
    . preg_replace(
        '/[^A-Za-z0-9_-]/',
        '_',
        dds_pdf_clean($dds['dds_no'] ?? ('ID_' . $viewId))
    )
    . '.pdf';
