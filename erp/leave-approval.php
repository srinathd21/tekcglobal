...
if (isset($_GET["approved"])) {
    ...
    $pageMessageText = "Leave request approved successfully.";
} elseif (isset($_GET["rejected"])) {
    $pageMessageType = "success";
    $pageMessageText = "Leave request rejected successfully.";
...
$where = $status === "all" ? "1=1" : "lr.status = '" . mysqli_real_escape_string($conn, $status) . "'";

$rows = [];
$q = mysqli_query($conn, "
    SELECT lr.*, e.full_name, e.employee_code, mlt.leave_type_name, p.project_name
...
<head>
    ...
    <title>Leave Approval - TEK-C PMC Construction</title>
...
<section class="page-section p-3 p-lg-3">
    <div class="page-head-card mb-3">
        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between gap-3">
            <div><h1 class="h4 fw-bold mb-1">Leave Approval</h1><p class="text-muted-custom mb-0 small">Only users with approve permission can approve or reject leave.</p></div>
            <a href="leave-requests.php" class="btn btn-outline-secondary rounded-4 fw-bold btn-sm px-3">Back</a>
        </div>
    </div>
    ...
    <section class="card-ui overflow-hidden">
        <div class="overflow-auto thin-scrollbar p-3">
            <table class="project-table w-100">
                <thead><tr><th>Employee</th><th>Leave Type</th><th>Dates</th><th>Project</th><th>Reason</th><th>Status</th><th>Action</th></tr></thead>
                <tbody>
                    <?php foreach ($rows as $row): ?>
                        ...
                        <tr>
                            ...
                            <td><?= e($row["leave_type_name"] ?: $row["leave_type"]) ?></td>
                            <td><?= e(dmy($row["from_date"])) ?> to <?= e(dmy($row["to_date"])) ?><br><small><?= e($row["total_days"]) ?> days</small></td>
                            ...
                            <td>
                                <?php if ($row["status"] === "Pending"): ?>
                                    <button class="btn btn-sm btn-outline-success rounded-4 fw-bold" data-bs-toggle="modal" data-bs-target="#approvalModal" onclick='openApproval(<?= $json ?>,"Approved")'>Approve</button>
                                    <button class="btn btn-sm btn-outline-danger rounded-4 fw-bold" data-bs-toggle="modal" data-bs-target="#approvalModal" onclick='openApproval(<?= $json ?>,"Rejected")'>Reject</button>
                                <?php else: ?>-<?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (empty($rows)): ?>
                        <tr><td colspan="7" class="text-center text-muted-custom py-4">No leave requests found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
<div class="modal fade" id="approvalModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="api/process-leave-approval.php" method="POST" class="modal-content">
            <input type="hidden" name="leave_id" id="leave_id">
            <input type="hidden" name="action_status" id="action_status">
            <div class="modal-header px-4">
                <h5 class="modal-title fw-bold" id="approvalTitle">Leave Approval</h5>
...
<script>
function openApproval(row, status) {
    document.getElementById("leave_id").value = row.id;
    document.getElementById("action_status").value = status;
    document.getElementById("approvalTitle").textContent = status === "Approved" ? "Approve Leave" : "Reject Leave";
}
</script>
</body>
</html>